...
gatherSamples sample (accum, rest, (WaveInfo s wave cv False), amp) = (newAccum, newRest, (WaveInfo (s +fromIntegral sample) wave cv False),aRest)
    where 
...
gatherSamples sample (accum, rest, (WaveInfo s wave cv True), amp) = (newAccum, newRest, (WaveInfo (s+(fromIntegral sample)) wave cv False), aRest)
    where 
        newAccum = accum ++ (force values')
        --(values, _) = splitAt sample rest         
        (_,newRest') = splitAt (round s) (generateWave sample (WaveInfo 0 wave cv True))
